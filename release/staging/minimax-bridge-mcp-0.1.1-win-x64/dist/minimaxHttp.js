...
import { extensionFromMime, mimeFromExt, safeExt } from "./artifacts.js";
...
function looksLikeSuccess(status) {
...
function collectUrlValues(value, out = []) {
    if (typeof value === "string" && /^https?:\/\//i.test(value)) {
        out.push(value);
    }
    else if (Array.isArray(value)) {
        for (const item of value)
            collectUrlValues(item, out);
    }
    else if (value && typeof value === "object") {
        for (const nested of Object.values(value))
            collectUrlValues(nested, out);
    }
    return [...new Set(out)];
}
...
export class MiniMaxHttpClient {
    ...
    async bytesRequest(url, options = {}) {
    ...
    async uploadFile(fileOrUrl, purpose, isUrl) {
        ...
        if (!fileId)
            throw new MiniMaxApiError("MiniMax file upload response did not include file_id", { responseBody: parsed });
        return { file_id: fileId, raw: parsed, tempFile };
    }
    async downloadUrlAsArtifact(url, options) {
    ...
    async poll(options) {
        ...
        while (true) {
            ...
            if (options.isDone(value))
                return value;
            if (options.isFail(value))
                throw new MiniMaxApiError(options.failureMessage(value), { responseBody: value });
            if (!options.pollUntilDone)
                return value;
            if ((Date.now() - started) / 1000 >= options.maxWaitSeconds)
                return value;
    ...
    async listVoices(args) {
        ...
        if (voiceType !== "all" && voiceType !== "system")
            voices = [];
        if (language)
            voices = voices.filter((v) => v.language.toLowerCase().includes(language));
        if (query) {
    ...
    async textToAudioAsyncHttp(input) {
        ...
        if (!taskId)
            throw new MiniMaxApiError("MiniMax t2a_async_v2 response did not include task_id", { responseBody: created });
    ...
    async textToAudioWebSocket(input) {
        const text = getString(input.text);
        if (!text)
            throw new UserInputError("text_to_audio websocket requires text");
        const format = getString(input.format) || "mp3";
        ...
        const audio = await new Promise((resolve, reject) => {
            ...
            ws.on("message", (data) => {
                try {
                    ...
                    if (audioHex)
                        chunks.push(Buffer.from(audioHex, "hex"));
    ...
    async queryTextToAudio(args) {
        const input = asRecord(args);
        const taskId = getString(input.task_id);
        if (!taskId)
            throw new UserInputError("query_text_to_audio requires task_id");
    ...
    async voiceClone(args) {
        const input = asRecord(args);
        const voiceId = getString(input.voice_id);
        const file = getString(input.file);
        if (!voiceId || !file)
            throw new UserInputError("voice_clone requires voice_id and file");
    ...
    async textToImage(args) {
        ...
        if (!prompt)
            throw new UserInputError("text_to_image requires prompt");
        const payload = {
    ...
    async generateVideo(args) {
        ...
        if (!prompt && !firstFrameImage)
            throw new UserInputError("generate_video requires prompt or first_frame_image");
        const payload = {
            ...(prompt ? { prompt } : {}),
        ...
        if (!taskId)
            throw new MiniMaxApiError("MiniMax video_generation response did not include task_id", { responseBody: raw });
        if (getBoolean(input.async_mode, false)) {
    ...
    async queryVideoGeneration(args) {
        const input = asRecord(args);
        const taskId = getString(input.task_id);
        if (!taskId)
            throw new UserInputError("query_video_generation requires task_id");
        const raw = await this.poll({
    ...
    async videoTemplateGeneration(args) {
        const input = asRecord(args);
        const templateId = getString(input.template_id);
        if (!templateId)
            throw new UserInputError("video_template_generation requires template_id");
        const payload = {
        ...
        const taskId = this.taskIdOf(raw);
        if (!taskId)
            throw new MiniMaxApiError("MiniMax video_template_generation response did not include task_id", { responseBody: raw });
        if (getBoolean(input.async_mode, false)) {
    ...
    async queryVideoTemplateGeneration(args) {
        const input = asRecord(args);
        const taskId = getString(input.task_id);
        if (!taskId)
            throw new UserInputError("query_video_template_generation requires task_id");
        const raw = await this.poll({
    ...
    async lyricsGeneration(args) {
        const input = asRecord(args);
        const prompt = getString(input.prompt);
        if (!prompt)
            throw new UserInputError("lyrics_generation requires prompt");
    ...
    async musicGeneration(args) {
        const input = asRecord(args);
        const prompt = getString(input.prompt);
        if (!prompt)
            throw new UserInputError("music_generation requires prompt");
        const format = getString(input.format) || "mp3";
        const payload = {
    ...
    async musicCoverPreprocess(args) {
        const input = asRecord(args);
        const audioUrl = getString(input.audio_url);
        if (!audioUrl)
            throw new UserInputError("music_cover_preprocess requires audio_url");
...
//# sourceMappingURL=minimaxHttp.js.map